...
public class MortgageCalculatorService {
    ...
    public BigDecimal calculateMortgage(MortgageData md) {
        ...
        // Principal
        Long principal = md.getKaufpreis() + md.getZusaetzlicherFinanzierungsbedarf() -
                md.getEigenkapital();

        // Interest rate
        BigDecimal annualInterest = BigDecimal.valueOf(md.getInterestRate());
        BigDecimal monthlyInterest = annualInterest.divide(TWELVE, MC)
                .divide(BigDecimal.valueOf(100.0), MC);
        ...
        BigDecimal monthlyPayment = BigDecimal.valueOf(principal).multiply(zwischenergebnis).setScale(2, RoundingMode.HALF_EVEN);
